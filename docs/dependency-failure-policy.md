...
| Optional feature dependency | OpenAI-backed search, CloudWatch statistics source, external SMTP when not selected | Validate only when the feature is enabled or selected. | Disable the feature or return a feature-specific degraded response when safe. If selected for a critical flow, fail that flow explicitly. |
