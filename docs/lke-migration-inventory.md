...
| coturn | K8s Deployment/Service in staging; legacy reference was public-only VM | `3478/tcp+udp`, `5349/tcp`, relay UDP range when explicitly exposed | Config and TLS material | Runtime-generated LKE staging bridge deploys coturn as a Deployment/Service with internal ClusterIP by default; production public TURN needs approved exposure and scaling design | Public TURN remains outside HAProxy edge v1; `LKE_COTURN_SERVICE_TYPE=LoadBalancer` is available only for explicit exposure testing while Linode UDP/TCP behavior and relay range are validated | High | Prove LKE TURN data-plane behavior and rollback before production public exposure. |
