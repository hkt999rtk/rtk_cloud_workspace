...
| Public HTTP(S) | External HAProxy edge VM in TCP mode forwards public `443/TCP` to ingress-nginx NodePort; public `80/TCP` remains closed. |
| Public MQTT | External HAProxy edge VM in TCP mode forwards public `8883/TCP` to EMQX/MQTT NodePort. |
| DNS | Workspace LKE `--dns` should provision GoDaddy A records to HAProxy edge VM public IPs and use GoDaddy DNS-01 for ACME TLS issuance. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Video Cloud public API | `cmd/api` on `api` VM behind `edge` nginx | Public HTTPS via `edge:443`, private app `18080` | PostgreSQL, object/blob storage | Runtime-generated LKE staging bridge deploys Deployment + ClusterIP Service | HAProxy TCP passthrough to ingress-nginx NodePort; preserve `/healthz`, `/version`, API routes | Medium | Manual rollback is DNS back to previous known-good edge endpoint; no NodeBalancer fallback target. |
...
| EMQX MQTT | Docker Compose on `mqtt` VM | MQTT `1883`, MQTTS `8883`, dashboard private | Broker config, retained/session state if enabled | EMQX operator/StatefulSet, or external broker | HAProxy TCP passthrough to EMQX/MQTT NodePort for MQTTS; not normal HTTP-only Ingress | High | TODO: confirm retained messages/session persistence and cluster requirements. |
| coturn | public-only VM | `3478/tcp+udp`, `5349/tcp`, relay UDP range | Config and TLS material | Runtime-generated LKE staging bridge deploys coturn as a Deployment/Service with internal ClusterIP by default; production public TURN needs approved exposure and scaling design | TURN is outside HAProxy v1; public UDP/TCP relay exposure requires a separate edge design | High | Prove LKE TURN data-plane behavior and rollback before removing VM coturn fallback. |
...
| Public edge | VM-local nginx and certbot DNS-01 | Public HTTPS and mTLS SNI hostnames | Certificates on VM disk | HAProxy edge VM forwards TCP to ingress-nginx and EMQX NodePorts; ingress-nginx in LKE owns Kubernetes TLS Secret generated by workspace DNS-01 flow | HAProxy host install + systemd, no Docker; cert-manager/webhook remains a later operator-owned option | High | First rollback is manual DNS back to previous known-good edge endpoint; failover automation is deferred but artifacts must be multi-VM shaped. |
