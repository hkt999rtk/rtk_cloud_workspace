...
| Staging E2E | `scripts/run-staging-e2e.sh` and workspace orchestration | Operator/local runner | Generated fixtures/artifacts | Current wrapper is provider-aware for `linode` and `lke`; Kubernetes-aware smoke/E2E Job later | Artifact output remains redacted | Medium | LKE `provision/deploy` requires explicit container image env vars; TODO: define production LKE smoke Job after manifests are approved. |
