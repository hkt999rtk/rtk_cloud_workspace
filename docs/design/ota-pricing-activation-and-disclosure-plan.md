...
| P3 Product 歷史授權證據（原始碼已合併，環境未部署） | Account Manager 的受保護內部查詢可依 Brand Cloud、Product、revision 讀出不可變 grant，重新驗證包含 Logger retention 設定的 digest，回報 `ota` 是否選用及從該版建立至下一版建立的時間區間。Video Cloud 首次任務指派會讀取當下有效的 Product grant，將 revision／digest／授權時間寫入不可變 receipt、來源事件與 producer 月度快照；缺少證據拒絕建立可計費任務。既有任務在 Product 停用後仍可繼續，新任務則不得建立。已驗證下載 receipt 保留 URL 授權的同組證據。韌體上傳會在遠端 PUT 前把有效 Product grant 寫入不可變預約，成功或隔離的物件、write 來源事件與 producer 快照沿用原授權；崩潰後盤點恢復也從預約取回。本機與 CI 資料庫整合測試已通過。尚未併入正式環境；Account Manager 歷史查詢不能重建當時 Product active 狀態或裝置授權。storage 仍以 Product／月彙總，Billing 尚不能逐一驗證參與的物件原授權；Billing fact 也尚未直接傳遞 grant 欄位或逐筆向 Account Manager 查核。 |
