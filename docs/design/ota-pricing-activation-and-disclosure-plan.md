...
| P3 Product 歷史授權證據（原始碼已合併，環境未部署） | Account Manager 的受保護內部查詢可依 Brand Cloud、Product、revision 讀出不可變 grant，重新驗證包含 Logger retention 設定的 digest，回報 `ota` 是否選用及從該版建立至下一版建立的時間區間。Video Cloud 首次任務指派會讀取當下有效的 Product grant，將 revision／digest／授權時間寫入不可變 receipt、來源事件與 producer 月度快照；缺少證據拒絕建立可計費任務。既有任務在 Product 停用後仍可繼續，新任務則不得建立。已驗證下載 receipt 保留 URL 授權的同組證據。韌體上傳會在遠端 PUT 前把有效 Product grant 寫入不可變預約，成功或隔離的物件、write 來源事件與 producer 快照沿用原授權；崩潰後盤點恢復也從預約取回。storage 封存前會拒絕原授權或相符已完成上傳預約缺失的物件，整個月結交易不產生 storage fact。本機與 CI 資料庫整合測試已通過。尚未併入正式環境；Account Manager 歷史查詢不能重建當時 Product active 狀態或裝置授權。storage 仍以 Product／月彙總，Billing 尚不能逐一驗證參與的物件原授權；Billing fact 也尚未直接傳遞 grant 欄位或逐筆向 Account Manager 查核。 |
...
| 稅務與 Product 適用 | [價卡資料模型](../../repos/rtk_billing/internal/billing/types.go) 與 [遷移](../../repos/rtk_billing/migrations/064_invoice_total_tax_policy.sql) 已支援 `invoice_total` 模式：各 line 先算未稅小計，所有服務合計後計稅一次並確定性分攤至 line；既有 `line` 模式供舊版保留。`tax_rate_basis_points=0` 不能代表 OTA 免稅。[有效價卡選擇](../../repos/rtk_billing/internal/billingstore/pricing.go) 仍只看時間／幣別，尚無逐筆 Product grant 條件。Account Manager 已有歷史 revision 查詢；Video Cloud 已把任務指派、已驗證下載及韌體物件建立的原始 grant 證據保存到各自的來源紀錄，物件亦在 producer 月度快照中保留證據。Billing fact 目前只有其來源雜湊，未直接包含 grant 欄位，Billing 也尚未查核。 | 新 OTA 價卡仍需審核過的**帳單層**稅率／分類與完整價卡審核紀錄；所有選價／估算／關帳／客戶 API 需使用同一稅務規則。OTA producer 須把四項 meter 的原授權 Product、revision、digest、授權時間傳入 Billing 可驗證的證據；storage Product／月聚合須讓 Billing 驗證每個參與物件的原授權。Billing 比對 Account Manager 歷史 revision 與 source 狀態，缺失或衝突一律不自動收費。關閉後的舊任務／儲存沿用其原授權。Finance／法務仍須核對發票設定。 |
