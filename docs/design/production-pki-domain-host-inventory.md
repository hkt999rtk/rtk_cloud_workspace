...
| Internal service client/server identity | The registry owns approved Service client identities, receipts, verification, reconciliation and revocation. Factory enrollment owns its verified outbound lifecycle; certificate issuer listener admission, eviction and exact CRL consumption are integrated. The existing Account Manager PKI owner can also terminate a separate internal mTLS listener, retain its server key, renew through its managed client and enforce exact API/factory route ACLs before loopback proxying. | Dev rollout of the Account Manager listener and managed API caller, then adoption by remaining Service hosts, dynamic root-policy/key renewal and live eviction, matched restore and post-backup audit reconciliation evidence. |
