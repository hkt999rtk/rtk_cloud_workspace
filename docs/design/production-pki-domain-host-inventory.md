...
Reviewed 2026-09-08 against workspace `76e1e13` and Video Cloud `7e0aaf2`.
...
| Gateway/server issuance | `internal/certissuer/service.go:handleGatewayIssue` validates a configured DNS allowlist. `signGateway` calls the same `s.signer` used by legacy Device issuance. `internal/certissuer/material.go:SignServer` changes role/EKU but retains that signer/mount. | Independent server-domain registry selection, approved SAN policy, durable claims/replay/recovery, publication and consumer adoption. Selecting another role in the Device mount does not meet domain separation. |
...
`f304ec1`; provider/HTTP composition and uncertain-outcome recovery remain.
Gateway runtime adoption is unfinished.
