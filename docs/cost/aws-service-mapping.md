...
| --- | --- | --- |
| Current private-cloud BOM | `../private-cloud-deployment.md` | Component inventory, single-node and production-like profiles, infrastructure expectations. |
| Cross-repo boundaries | `../architecture.md` | Source-of-truth repository ownership and service boundaries. |
...
- Video Cloud supports token signing through an `ed25519-secret`, PEM key files,
  or a PKCS#11 provider. SoftHSM2 is the local/CI PKCS#11 target; production
  should use AWS CloudHSM or another supported production HSM module.
- OpenHSM is not part of the current implementation or deployment model.
...
| Required AWS support tier and account structure | AWS Support monthly plan fees. |
| Launch, migration, or consulting scope | AWS Countdown Premium and Marketplace Professional Services quote items. |
...
| Support/quote treatment | Recurring support plan, one-time/project consulting, quote-required, or excluded. |
...
7. Decide whether TURN relay is self-operated on AWS or delegated to a managed
   TURN provider.
8. Decide the support tier: Business Support+ as the default production support
   adder, or Enterprise/Unified Operations when TAM coverage or mission-critical
   escalation is required.
9. Decide whether consulting is an AWS Marketplace Professional Services private
   offer, an AWS partner quote, or outside the AWS bill.
