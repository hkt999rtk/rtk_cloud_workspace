...
- Target：2026-08-01 pass 100,000 IoT devices + 5,000 video cameras loading test。
...
- Milestone detail table：只作為輔助明細，內容包含 May kickoff、May foundation、late-May/early-June load-test preparation、June small/medium validation、late-June multi-host/capacity、July 10k/30k rehearsal、late-July 100k dry run、early-Aug final pass。
...
- HSM / PKCS#11 signer design：說明 certissuer CA signing 與 Ed25519 token signing 的 key custody boundary；service 只取得 signing capability，不持有 raw private key material。
