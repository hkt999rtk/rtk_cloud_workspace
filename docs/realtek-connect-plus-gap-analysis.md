...
| Cross-service provisioning channel | `deferred`, `not-current-runtime` | Shared broker packaging is retired; account/video lifecycle coordination should use explicit service APIs and DB-backed outbox/retry if async delivery is reintroduced. | Need a future eventing requirement before adding a broker back. |
...
| MQTT transport and broker | `implemented`, `foundation-gap` | Video cloud has MQTT adapter and EMQX reference broker packaging. | Remaining work is operator evidence and service-local smoke/runbook depth. |
