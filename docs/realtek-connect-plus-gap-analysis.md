...
| WebRTC / TURN | `implemented`, `migration-planned` | Server-side signaling, TURN credential policy, and TURN registry control plane exist. Product direction is WebRTC Video over TURN only; legacy RTSP relay and legacy video relay are breaking-removal targets. | Track removal through `docs/webrtc-only-streaming-migration.md`; do not open new RTSP/legacy relay enhancement work. |
