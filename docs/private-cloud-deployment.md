...
| Central service logger | Yes for production-like profile | `rtk_cloud_logger` with workspace provisioning | Required target: Loki-backed logger backend, journald forwarder, and ingest/query API. Staging native provisioning is pending. | Stores queryable service logs across account, video, admin, frontend, and non-Go host sources without requiring Grafana for the v1 dashboard. |
