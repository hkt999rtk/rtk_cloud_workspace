...
| Central service logger | Yes for production-like profile | `rtk_cloud_logger` with workspace provisioning | Implemented in staging native flow: logger resource provisioning, Loki-backed backend service, journald forwarders, readiness checks, artifacts, cleanup, and Cloud Admin dashboard wiring. | Stores queryable service logs across account, video, admin, frontend, and non-Go host sources without requiring Grafana for the v1 dashboard. |
