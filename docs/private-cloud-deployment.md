...
| Central service logger | Yes for production-like profile | `rtk_cloud_logger` with workspace provisioning | Partially implemented in staging: logger resource provisioning, forwarders, readiness, artifacts, cleanup, Loki-backed store, and Cloud Admin dashboard are present; native logger VM backend/Loki service install still needs verification/follow-up. | Stores queryable service logs across account, video, admin, frontend, and non-Go host sources without requiring Grafana for the v1 dashboard. |
