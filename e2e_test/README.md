...
| `../scripts/staging_email_signup_e2e.py` | Deployed staging customer signup, SMTP delivery, IMAP receipt, and browser activation. | Implemented; explicit operator opt-in only. |
...
## Cloud SMTP + IMAP signup activation
...
It is intentionally not a CI/nightly test because the operator-held SMTP and
IMAP credentials must remain in `~/.env`.
...
The command maps only the process-local SMTP aliases from `~/.env` to the LKE
runtime variables, deploys the Account Manager/email worker configuration, and
writes redacted evidence to `.artifacts/e2e_test/email-signup/<run-id>/`.
Use `--skip-deploy` only after the same SMTP configuration has already rolled
out successfully.
