...
func TestRefreshRepoEnrichesRunAndUsesETag(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/repos/hkt999rtk/repo/actions/runs":
			runRequests.Add(1)
			if r.Header.Get("If-None-Match") == `"runs-v1"` {
				w.WriteHeader(http.StatusNotModified)
				return
			}
			w.Header().Set("ETag", `"runs-v1"`)
			fmt.Fprint(w, `{"workflow_runs":[{"id":77,"name":"CI","display_title":"Fix retries","event":"pull_request","status":"in_progress","workflow_id":9,"run_number":12,"run_attempt":2,"head_branch":"fix/retry","head_sha":"1234567890","html_url":"https://github.test/run/77","created_at":"2026-09-01T01:00:00Z","updated_at":"2026-09-01T01:02:00Z","run_started_at":"2026-09-01T01:00:05Z","actor":{"login":"amy"},"triggering_actor":{"login":"bob"},"pull_requests":[{"number":42}]}]}`)
		...
		case "/repos/hkt999rtk/repo/actions/runs/77/jobs":
			fmt.Fprint(w, `{"jobs":[{"id":1,"name":"test","status":"completed","conclusion":"failure","steps":[]},{"id":2,"name":"build","status":"in_progress","conclusion":null,"steps":[]}]}`)
		default:
			http.NotFound(w, r)
		}
	}))
	...
	if len(snapshot.Running) != 1 {
		t.Fatalf("running = %d, want 1", len(snapshot.Running))
	}
	card := snapshot.Running[0]
	...
	if runRequests.Load() != 2 || len(p.snapshot().Running) != 1 {
		t.Fatal("304 refresh should preserve cached run")
	}
