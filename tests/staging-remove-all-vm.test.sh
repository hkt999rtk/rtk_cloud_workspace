...
if PATH="$FAKE_BIN:$PATH" CURL_LOG="$LOG" CURL_STATE="$CURL_STATE" LINODE_TOKEN=test-token \
	"/usr/local/go/bin/go" run "$ROOT/scripts/go/rtk-cloud" -- remove-all-vm --workspace "$WORKSPACE" >/dev/null 2>"$TMP/missing-env-root.err"; then
...
printf 'no\n' | PATH="$FAKE_BIN:$PATH" CURL_LOG="$LOG" CURL_STATE="$CURL_STATE" LINODE_TOKEN=test-token \
	"/usr/local/go/bin/go" run "$ROOT/scripts/go/rtk-cloud" -- remove-all-vm --workspace "$WORKSPACE" --env-root "$ENV_ROOT" >/dev/null
if [[ -e "$LOG" ]]; then
