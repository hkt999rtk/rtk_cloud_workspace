...
Options:
  ...
  --admin-release VERSION             Optional; otherwise use rtk_cloud_admin git short SHA.
...
list_video_release_manifest_keys() {
	object_storage_aws ls "s3://$LINODE_OBJ_BUCKET/releases/" --recursive \
		| awk '$4 ~ /^releases\/rtk_video_cloud-[^\/]+\/manifest\.json$/ {print $1 " " $2 "\t" $4}' \
		| sort -r
}

version_from_manifest_key() {
	local key="$1"
	key="${key#releases/rtk_video_cloud-}"
	printf '%s\n' "${key%/manifest.json}"
}

select_video_release_manifest_key() {
	local list_file count i line modified key version choice
	list_file="$(mktemp /tmp/rtk-video-releases.XXXXXX)"
	list_video_release_manifest_keys > "$list_file"
	count="$(wc -l < "$list_file" | tr -d ' ')"
	if [[ "$count" == "0" ]]; then
		rm -f "$list_file"
		die "no rtk_video_cloud release manifest found in Object Storage under releases/"
	fi
	printf 'Available rtk_video_cloud releases in Object Storage:\n' >&2
	i=1
	while IFS=$'\t' read -r modified key; do
		version="$(version_from_manifest_key "$key")"
		printf '  %d) %s  %s\n' "$i" "$version" "$modified" >&2
		i=$((i + 1))
	done < "$list_file"
	if [[ -t 0 ]]; then
		printf 'Select release [1]: ' >&2
		read -r choice
		choice="${choice:-1}"
	else
		choice=1
		log "non-interactive shell; selecting newest Object Storage release"
	fi
...
check_latest_video_release() {
	...
	if [[ -n "$VIDEO_RELEASE" ]]; then
		manifest_key="releases/rtk_video_cloud-$VIDEO_RELEASE/manifest.json"
	else
		manifest_key="$(select_video_release_manifest_key)"
	fi
	...
	[[ -z "$VIDEO_RELEASE" || "$VIDEO_RELEASE" == "$version" ]] || die "manifest version mismatch: requested $VIDEO_RELEASE but $manifest_key contains $version"
	VIDEO_RELEASE="$version"
	log "selected Object Storage release: $version"
	object_storage_aws ls "s3://$LINODE_OBJ_BUCKET/$object_key" >/dev/null
	log "Object Storage release readable: $object_key"
}
...
resolve_deploy_releases() {
	...
	if [[ -z "$ADMIN_RELEASE" ]]; then
		ADMIN_RELEASE="$(git_short_sha "$ADMIN_REPO")"
	fi
	...
	for attempt in $(seq 1 5); do
		if (
			...
			GODADDY_ENV="$GODADDY_ENVIRONMENT" go run ./cmd/godaddy-dns \
				...
				records upsert "$DNS_ROOT_DOMAIN" --type A --name "$name" --data "$ip" --ttl 600
		); then
			return 0
		fi
		log "GoDaddy upsert failed for $domain attempt $attempt/5; retrying in ${delay}s"
		sleep "$delay"
		delay=$((delay * 2))
	done
	return 1
...
dns_stack() {
	...
	godaddy_upsert_a "$VC_GATEWAY_DOMAIN" "$edge_ip"
	godaddy_upsert_a "$VC_CERTISSUER_DOMAIN" "$edge_ip"
	godaddy_upsert_a "$AM_DOMAIN" "$ACCOUNT_MANAGER_LINODE_PUBLIC_IPV4"
	godaddy_upsert_a "$ADMIN_DOMAIN" "$ADMIN_LINODE_PUBLIC_IPV4"
...
deploy_stack() {
	...
	bash "$STAGING_DEPLOY_SCRIPT" "${args[@]}"
	log "deploy complete"
}
