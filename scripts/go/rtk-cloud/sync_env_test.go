...
func TestSyncEnvCheckAllowsLKEWithoutTopology(t *testing.T) {
	...
	mkdirAll(t, filepath.Join(envRoot, "services", "cloud-logger"))
...
`)
	...
	writeFile(t, filepath.Join(envRoot, "services", "cloud-logger", "logger.env"), `CLOUD_LOGGER_DOMAIN=logger.video-cloud-staging.realtekconnect.com
CLOUD_LOGGER_ENDPOINT=https://logger.video-cloud-staging.realtekconnect.com
`)

	if err := run([]string{"sync-env", "--workspace", workspace, "--env-root", envRoot, "--check"}); err != nil {
		t.Fatalf("sync-env --check returned error: %v", err)
	}
}
...
`)
	...
	logger := readFile(t, filepath.Join(envRoot, "services", "cloud-logger", "logger.env"))
	for _, want := range []string{
		"CLOUD_LOGGER_DOMAIN=logger.video-cloud-staging.realtekconnect.com",
		"CLOUD_LOGGER_ENDPOINT=https://logger.video-cloud-staging.realtekconnect.com",
	} {
