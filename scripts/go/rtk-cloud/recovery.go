...
func recoveryMutationGuard(args []string) error {
	...
	switch args[0] {
	case "base-backup", "wal-restore", "wal-archive", "backup", "restore", "docs-check", "contracts-check", "status-all", "test-catalog", "test-inventory", "test-spec-inventory", "test-spec-impact", "test-coverage", "test-coverage-aggregate", "test-feature-coverage":
		return nil
	}
