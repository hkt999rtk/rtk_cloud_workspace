...
func loadAndValidateTestCatalogForRunner(workspace, sourceRunner string) (testCatalog, error) {
	...
	if catalog.SchemaVersion != 3 {
		return testCatalog{}, fmt.Errorf("test catalog schema_version=%d, want 3", catalog.SchemaVersion)
	}
...
func validateFeatureRequirements(workspace string, catalog testCatalog, sourceRunner string) error {
	...
	for i, feature := range catalog.Features {
		...
		if feature.Status != "active" && feature.Status != "retired" {
		...
		for j, requirement := range feature.Requirements {
			...
			if requirement.Status != "active" && requirement.Status != "retired" {
...
func renderTestCatalog(catalog testCatalog) []byte {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "| Feature ID | Feature | Risk | Requirement ID | Requirement | Layer | Gate | Freshness | Owner | Status |")
	fmt.Fprintln(&b, "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |")
	...
	for _, feature := range features {
		...
		for _, requirement := range requirements {
			...
			fmt.Fprintf(&b, "| `%s` | %s | `%s` | `%s` | %s | `%s` | `%s` | %s | `%s` | `%s` |\n",
				...
				escapeMarkdownCell(requirement.Title), requirement.AcceptanceLayer, requirement.Gate,
				freshness, feature.Owner, requirement.Status)
		}
	}
	fmt.Fprintln(&b)
