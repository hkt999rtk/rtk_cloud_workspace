...
func logsCheckRemoteSpecs(cfg logsCheckConfig) []logsCheckRemoteSpec {
	...
	return []logsCheckRemoteSpec{
		...
		{"api-journal", "api", "journalctl -u video_cloud-api.service -u video_cloud-logingester.service -u video_cloud-turnregistry.service -u video_cloud-metricsexporter.service -u video_cloud-cleaner.service -u video_cloud-statistics.service -u video_cloud-certissuer.service -u video_cloud-factoryenroll.service --since " + since + " -n " + tail + " --no-pager || true", "api-journal.log"},
		{"infra-services", "infra", serviceStatusCommand([]string{"postgresql", "redis-server", "prometheus"}, nil), "infra-services.log"},
		{"infra-journal", "infra", "journalctl -u postgresql -u redis-server -u prometheus --since " + since + " -n " + tail + " --no-pager || true", "infra-journal.log"},
