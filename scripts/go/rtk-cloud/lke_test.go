...
func TestRunLKEResolveImagesWritesPinnedSubmoduleManifest(t *testing.T) {
	...
	for _, want := range []string{
		...
		`"LKE_BILLING_IMAGE": "ghcr.io/hkt999rtk/rtk_billing/billing-twd:sha-` + commits["rtk_billing"] + `"`,
...
func TestRunProvisionLKEDeployAppliesRuntimeDependencies(t *testing.T) {
	...
	for _, want := range []string{
		...
		"SOCIAL_LOGIN_CALLBACK_URL: \"https://admin.video-cloud-staging.realtekconnect.com/api/auth/social/callback\"",
...
func TestRestrictPrivateE2EArtifacts(t *testing.T) {
	...
	deviceDir := filepath.Join(envRoot, "devices", "test_device")
	if err := os.MkdirAll(deviceDir, 0o755); err != nil {
		t.Fatal(err)
	}
	deviceFile := filepath.Join(deviceDir, "loadtest.env")
	if err := os.WriteFile(deviceFile, []byte("DEVICE_ID=test\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := restrictPrivateE2EArtifacts(envRoot, outDir); err != nil {
		t.Fatal(err)
	}
	for _, path := range []string{filepath.Join(envRoot, "artifacts"), outDir, file, filepath.Join(envRoot, "devices"), deviceDir, deviceFile} {
		info, err := os.Stat(path)
		if err != nil {
			t.Fatal(err)
		}
...
func TestAccountManagerDockerfileIncludesMigrateBinaryAndMigrations(t *testing.T) {
	...
	for _, want := range []string{
		...
		"go build -trimpath -o /out/rtk-account-manager-cloud-deletion-worker ./cmd/cloud-deletion-worker",
		"apt-get install -y --no-install-recommends ca-certificates",
