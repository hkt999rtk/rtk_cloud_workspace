...
func TestSharedKubernetesPlacementUsesPerWorkloadNodeClasses(t *testing.T) {
	...
	env := map[string]string{
		...
		"FLEET_VALKEY_NODE_CLASS": "database", "FLEET_VALKEY_EXPORTER_NODE_CLASS": "observability",
	}
	...
	for _, want := range []string{
		...
		"-n video-cloud-dev-platform patch statefulset fleet-valkey",
		"-n video-cloud-dev-platform patch deployment fleet-valkey-exporter",
		"-n video-cloud-dev-platform rollout status statefulset/fleet-valkey",
		"-n video-cloud-dev-video-cloud patch statefulset mqtt",
		`rtk.io/node-class":"general`,
		`rtk.io/node-class":"database`,
		`rtk.io/node-class":"observability`,
		`effect":"NoSchedule","key":"rtk.io/node-class","operator":"Equal","value":"database`,
	} {
