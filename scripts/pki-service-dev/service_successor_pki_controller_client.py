#!/usr/bin/env python3
"""Rotate only pki-controller's Dev Service client to the final authority."""
import argparse
import copy
import fcntl
...
class PkiControllerFinalClient(r.ServiceRun):
    def __init__(self, args):
        super().__init__(args)
        self.report['foundation_scope'] = 'Dev pki-controller final Service client leaf rotation; predecessor server retained for R4'
        self.report['service_successor_pki_controller_client_sha256'] = m.digest(Path(__file__).read_bytes())
    ...
    def issuer(self):
        ...
        m.require(value['status'] == 'active' and value['environment'] == 'dev'
                  ...
                  'final pki-controller client authority changed')
    ...
    def rotate(self):
        ...
        m.require(before['client_rows'] and before['server_rows']
                  ...
                  'pki-controller final client rotation baseline differs')
        ...
        while True:
            ...
            m.require(after['server_rows'] == before['server_rows'],
                      'pki-controller client-only renewal changed the predecessor server identity')
            m.require(time.monotonic() < deadline, 'pki-controller client-only renewal incomplete; retain evidence and do not signal again')
            time.sleep(2)
        m.require(added_client[0]['issuer_id'] == FINAL and added_client[0]['subject'] == SUBJECT
                  ...
                  and after['server_rows'] == before['server_rows'],
                  'pki-controller final client registry result differs')
        ...
        template.setdefault('metadata', {}).setdefault('annotations', {})['rtk.cloud/r2-pki-controller-final-client'] = self.output.name
        ...
        m.require(self.state() == after['client'] and self.rows('client') == after['client_rows']
                  and self.rows('server') == before['server_rows'], 'pki-controller restart changed final client-only state')
        factory.FactoryIdentityRun.factory_canary(self)
        self.check('pki_controller_final_service_client', {
            'final_issuer_id': FINAL, 'client_request_id': added_client[0]['request_id'],
            'predecessor_server_retained_for_r4': True, 'restart_preserved_identity': True,
...
def main():
    ...
    args = parser.parse_args(); args.phase = 'pki-controller-final-service-client'; args.authority = None
    lock = Path(args.config_root).expanduser() / 'dev/pki/service-rollout.lock'
    fd = os.open(lock, os.O_RDWR | os.O_CREAT | os.O_NOFOLLOW, 0o600); fcntl.flock(fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
    runner = PkiControllerFinalClient(args)
    try:
