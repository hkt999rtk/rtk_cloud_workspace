...
Staging 的主要編排腳本。它可以做 preflight、plan、reset、apply、DNS、deploy、artifact collection、e2e smoke。`CLOUD_PROVIDER=linode` 走既有 Linode VM path；`CLOUD_PROVIDER=lke` 走 kubectl path，建立/刪除 namespaces、套用 runtime-generated base resources，並在 `deploy` 時要求明確的 container image env vars。預設不變更環境，只做 `--plan`。
...
Staging 一站式整合測試編排腳本。它把 provider resource removal、provision all、staging E2E data setup，以及 home MQTT simulation 串成單一流程，最後輸出 sanitized `summary.json` 與 `TEST_REPORT.md`。Linode provider 會 remove VM/firewall/VPC；LKE provider 會刪除對應 stack namespaces。建立 RTK brand cloud、建立測試 users、產生並 factory-enroll devices、device bind/provision、bulk bind validation 已拆到 `scripts/setup-staging-e2e-data.sh` / `rtk-cloud staging-e2e-data-setup`，完整 E2E 會呼叫這個獨立步驟。
